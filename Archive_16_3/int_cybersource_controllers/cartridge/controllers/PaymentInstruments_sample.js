...
function create() {
	...
	if (enableTokenization=='YES') {    
	    ...
	    if (createSubscriptionMyAccountResult.error) {
	    	SubscriptionError = createSubscriptionMyAccountResult.reasonCode + "-" + createSubscriptionMyAccountResult.decision;
	    	return {
	            error: true,
	            SubscriptionError: SubscriptionError
	    	};
    ...
    for (var i = 0; i < paymentInstruments.length; i++) {
        var card = paymentInstruments[i];
        if (card.creditCardExpirationMonth === newCreditCardForm.get('expiration.month').value() && card.creditCardExpirationYear === newCreditCardForm.get('expiration.year').value()
        		&& card.creditCardType === newCreditCardForm.get('type').value() && card.getCreditCardNumber().indexOf(ccNumber.substring(ccNumber.length-4))) {
            isDuplicateCard = true;
            oldCard = card;
            break;
        }
