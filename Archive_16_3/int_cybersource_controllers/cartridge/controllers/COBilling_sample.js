...
function resetPaymentForms() {
    ...
    var status = Transaction.wrap(function () {
        if (app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals('PayPal')) {
            ...
            cart.removePaymentInstruments(cart.getPaymentInstruments("ALIPAY"));
       ...
       } else if(app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals("ALIPAY")){
        	app.getForm('billing').object.paymentMethods.creditCard.clearFormElement();
            app.getForm('billing').object.paymentMethods.bml.clearFormElement();
...
function handlePaymentSelection(cart) {
    ...
    if(result.OK && app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value =='PayPal' && !empty(result.paypalRedirectUrl))
    {
    	response.redirect(result.paypalRedirectUrl);
    }
    ...
    app.getForm('billing').handleAction({
        ...
        save: function () {
            ...
            handlePaymentSelection(cart).error) {// Performs payment method specific checks, such as credit card verification.
                if(app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value =='PayPal'){
                	
                	returnToForm(cart,{PaypalSetServiceError:true});
                }else{
                	returnToForm(cart);
                }
            } else {
...
function selectCreditCard() {
    ...
    if (request.httpParameterMap.creditCardUUID.value && applicableCreditCards && !applicableCreditCards.empty) {
        ...
        if (selectedCreditCard) {
            ...
            app.getForm('billing').object.paymentMethods.creditCard.subscriptionToken.value = selectedCreditCard.getCreditCardToken();
        }
    }
...
function saveCreditCard() {
    ...
    var cart = app.getModel('Cart').get();
    if(app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals(PaymentInstrument.METHOD_BML) || 
    		app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals("PayPal") || 
    		app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals("ALIPAY") || 
    		!empty(cart.getPaymentInstruments("PayPal"))) {
    	return true;
    }
    if (customer.authenticated && app.getForm('billing').object.paymentMethods.creditCard.saveCard.value) {
    	...
    	if (empty(app.getForm('billing').object.paymentMethods.creditCard.subscriptionToken.value) && enableTokenization=='YES' ){
        ...
        if (!empty(subscriptionID)) {
        	app.getForm('billing').object.paymentMethods.creditCard.subscriptionToken.value = createSubscriptionBillingResult.subscriptionID;
        }
        Transaction.wrap(function () {
        	...
        	if(!empty(app.getForm('billing').object.paymentMethods.creditCard.subscriptionToken.value)) {
        		newCreditCard.setCreditCardToken(app.getForm('billing').object.paymentMethods.creditCard.subscriptionToken.value);
        	}
