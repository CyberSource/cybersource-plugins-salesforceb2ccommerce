'use strict';

var assert = require('chai').assert;
var proxyquire = require('proxyquire').noCallThru().noPreserveCache();
var serverMock = require('../../../../../mocks/server');
var orderMock = require('../../../../../mocks/models/order');
var siteMock = require('../../../../../mocks/dw/system/Site');
var loggerMock = require('../../../../../mocks/dw/system/Logger');
var statusMock = require('../../../../../mocks/dw/system/Status');
var transactionMock = require('../../../../../mocks/dw/system/Transaction');
var PaymentMgrMock = require('../../../../../mocks/dw/order/PaymentMgr');
var CybersourceConstants = require('../../../../../../cartridges/LINK_cybersource/cartridge/scripts/utils/CybersourceConstants');

orderMock.paymentInstrument = {
    paymentTransaction: {
        paymentProcessor: 'cybersource'
    }
};

PaymentMgrMock.getPaymentMethod = function () {
    return {
        getPaymentProcessor: function () {
            return 'cybersource';
        }
    };
};

var applePayResponseData = {
    isTrusted: true,
    payment: {
        billingContact: {
            addressLines: ['35 Broad St'],
            administrativeArea: 'NJ',
            country: 'United States',
            countryCode: 'us',
            familyName: 'Doe',
            givenName: 'John',
            locality: 'Red Bank',
            postalCode: '07701',
            subAdministrativeArea: '',
            subLocality: ''
        },
        shippingContact: {
            addressLines: ['35 Broad St'],
            administrativeArea: 'NJ',
            country: 'United States',
            countryCode: 'us',
            emailAddress: 'tester@cybersource.com',
            familyName: 'Doe',
            givenName: 'John',
            locality: 'Red Bank',
            phoneNumber: '7329825689',
            postalCode: '07701',
            subAdministrativeArea: '',
            subLocality: ''
        },
        token: {
            paymentData: {},
            paymentMethod: {},
            transactionIdentifier: ''
        }
    }
};

global.webreferences = function () {
    return {
        CyberSourceTransaction: {}
    };
};
global.request = {
    httpParameterMap: {
        requestBodyAsString: '{"isTrusted":true,"payment":{"billingContact":{"addressLines":["35 Broad St"],"administrativeArea":"NJ","country":"United States","countryCode":"us","familyName":"Liotta","givenName":"Chris","locality":"Red Bank","postalCode":"07701","subAdministrativeArea":"","subLocality":""},"shippingContact":{"addressLines":["35 Broad St"],"administrativeArea":"NJ","country":"United States","countryCode":"us","emailAddress":"tester@cybersource.com","familyName":"Liotta","givenName":"Chris","locality":"Red Bank","phoneNumber":"7329825689","postalCode":"07701","subAdministrativeArea":"","subLocality":""},"token":{"paymentData":{"version":"EC_v1","data":"","signature":"","header":{"ephemeralPublicKey":"MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAE+PQpRgljisTlXbLVaS2x159WDSVqzkI8aXVOLNdFO2TcUlNWYm9QpTqnP1JvKtUzBh2v3JSQTUwwZBnjCBIADg==","publicKeyHash":"72aHd+5+77rSpwOmJtoF+6ne4/Yq8N5i1c/4qchR3P0=","transactionId":"86eb6e3fd273b7bfb918e9546ab042c7aba1ee7d9e0908448b39d04d773f0777"}},"paymentMethod":{"displayName":"Discover 2780","network":"Discover","type":"credit"},"transactionIdentifier":"86EB6E3FD273B7BFB918E9546AB042C7ABA1EE7D9E0908448B39D04D773F0777"}}}'
    }
};
global.empty = function (variable) {
    var isEmpty = true;
    if (variable) {
        isEmpty = false;
    }
    return isEmpty;
};

describe('Apple pay auth hook', function () {
    var CardHelper = proxyquire('../../../../../../../int_cybersource_sfra/cartridges/LINK_cybersource/cartridge/scripts/helper/CardHelper.js', {
        'dw/system/Logger': loggerMock,
        '~/cartridge/scripts/utils/CybersourceConstants': CybersourceConstants,
        'dw/system/Site': siteMock
    });

    var MobilePaymentsAdapter = {
        processPayment: function () {
            var authResponseStatus = {
                ServiceResponse: {
                    serviceResponse: 100
                }
            };
            return authResponseStatus;
        }
    };

    var applepayAuth = proxyquire('../../../../../../cartridges/LINK_cybersource/cartridge/scripts/hooks/applepay/applePayAuth', {
        'server': serverMock,
        '*/cartridge/scripts/utils/CybersourceConstants': CybersourceConstants,
        '~/cartridge/scripts/mobilepayments/adapter/MobilePaymentsAdapter': MobilePaymentsAdapter,
        'dw/system/Status': statusMock,
        '~/cartridge/scripts/helper/CardHelper': CardHelper,
        'dw/system/Transaction': transactionMock,
        'dw/order/PaymentMgr': PaymentMgrMock
    });

    it('should return a success status', function () {
        var applePayAuthStatus = applepayAuth.authorizeOrderPayment(orderMock, applePayResponseData);

        assert.equal(applePayAuthStatus.status, statusMock.OK);
    });
});
