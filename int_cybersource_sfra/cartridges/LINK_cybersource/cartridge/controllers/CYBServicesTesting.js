 ...
 * With the hard coded card data, just to verify service is working.
 */
server.get('TestFingerprint', server.middleware.https, function (req, res, next) {
    res.render('services/TestFingerPrintRedirect');
    next();
});
...
function TestPOS(args) {
    ...
    if (empty(request.httpParameterMap.service.stringValue)) {
        ...
        var result = POSAdaptor.InitiatePOSAuthRequest({
            storeLocation: storeLocation, entryMode: entryMode,
...
server.get('StartServices', server.middleware.https, function (req, res, next) {
