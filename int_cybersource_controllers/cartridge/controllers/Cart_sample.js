...
function submitForm() {
    ...
    formResult = cartForm.handleAction({
        ...
        'paypalExpressCheckout':function(){
        	...
        	var Cybersource = require('int_cybersource_controllers/cartridge/controllers/Cybersource_Services')
    	  	return	Cybersource.PaypalExpressCheckout(cart.object);
...
function miniCartExpressCheckout(){
	  var cart = app.getModel('Cart').get();
	  	Transaction.wrap(function() {
	  		cart.calculate();
	  	});
...
exports.MiniCartExpressCheckout = guard.ensure(['https'],miniCartExpressCheckout);
