...
function GetRequestDataForSilentPost() {
	...
	if (cart) {
		...
		if (isValid && (result.success)) {
			...
			result.errorMsg = Resource.msg('silentpost.invalidRequestData.error', 'checkout', null);
		}
	}
	}
...
function TestSATokenCreate(args) {
	...
	app.getView({
		log : !empty(result.errorMsg) ? result.errorMsg : Resource.msg('silentpost.testCreateToken.error', 'checkout', null)
