...
function addProductAndSubmitWeChatPay() {
	...
	before(function () {
		return request(myRequest)
			...
			.then(function (csrfResponse) {
				return WeChatPay(csrfResponse, myRequest);
			})
			.then(function (csrfResponse) {
	...
	it('expect to add to cart and submit WeChatPay to succeed', function () {
		return request(myRequest)
...
function WeChatPay(csrfResponse, myRequest) {
	...
	myRequest.form = {
		...
		dwfrm_billing_paymentMethod: 'WECHAT',
...
describe('WeChatPay from checkout', function () {

	describe('positive WeChatPay', function () {
		this.timeout(60000);
		return addProductAndSubmitWeChatPay();
	});
});
