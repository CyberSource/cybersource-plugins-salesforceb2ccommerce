...
function addProductAndSubmitKlarna() {
	...
	before(function () {
		return request(myRequest)
			...
			.then(function (csrfResponse) {
				return Klarna(csrfResponse, myRequest);
			})
			.then(function (csrfResponse) {
				var order = {orderNo:'1234567'};
                myRequest.method = 'GET';
				var order_id = '00012901';
                myRequest.url = config.baseUrl + '/COPlaceOrder-Submit?' + 'order_id'  + '=' + '00012901' + '&' + 'provider'  + '=' + 'KLARNA';
				var cookie = request.cookie(cookieString);
                cookieJar.setCookie(cookie, myRequest.order_id);
				return request(myRequest);
            });
	});
	it('expect to add to cart and submit Klarna to succeed', function () {
		return request(myRequest)
...
function Klarna(csrfResponse, myRequest) {
	...
	myRequest.form = {
		...
		dwfrm_billing_paymentMethod: 'KLARNA',
		dwfrm_billing_klarnaEmail: 'blahblah@gmail.com',
...
describe('Klarna from checkout', function () {

	describe('positive Klarna', function () {
		this.timeout(60000);
		return addProductAndSubmitKlarna();
	});
});
