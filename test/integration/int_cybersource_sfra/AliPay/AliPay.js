...
function addProductAndSubmitAliPay() {
	...
	before(function () {
		return request(myRequest)
			...
			.then(function (csrfResponse) {
				return AliPay(csrfResponse, myRequest);
			})
			.then(function (csrfResponse) {
				var order = {orderNo:'1234567'}; 
                myRequest.method = 'GET';
				var order_id = '00012901';
                myRequest.url = config.baseUrl + '/COPlaceOrder-Submit?' + 'order_id'  + '=' + '00012901' + '&' + 'provider'  + '=' + 'ALIPAY';
	...
	it('expect to add to cart and submit AliPay to succeed', function () {
		return request(myRequest)
...
function AliPay(csrfResponse, myRequest) {
	...
	myRequest.form = {
		...
		dwfrm_billing_paymentMethod: 'ALIPAY',
...
describe('AliPay from checkout', function () {

	describe('positive AliPay', function () {
		this.timeout(60000);
		return addProductAndSubmitAliPay();
	});
});
