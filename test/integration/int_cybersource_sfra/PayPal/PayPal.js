...
function addProductAndSubmitPayPal() {
	...
	before(function () {
		...
		return request(myRequest)
			...
			.then(function (csrfResponse) {
				return PayPal(csrfResponse, myRequest);
			})
			.then(function (csrfResponse) {
	...
	it('expect to add to cart and submit PAYPAL to succeed', function () {
		return request(myRequest)
			.then(function (response) {
			//var bodyAsJson = JSON.parse(response.body);
			//var strippedBody = jsonHelpers.deleteProperties(bodyAsJson, ['redirectUrl', 'action', 'queryString']);
			assert.equal(response.statusCode, 200, 'Expected to submit PAYPAL statusCode to be 200.');
...
function PayPal(csrfResponse, myRequest) {
...
describe('PayPal from checkout', function () {

	describe('positive PayPal', function () {
		this.timeout(60000);
		return addProductAndSubmitPayPal();
	});
});
