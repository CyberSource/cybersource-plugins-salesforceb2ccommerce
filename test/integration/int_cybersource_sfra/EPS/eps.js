...
function addProductAndSubmitEPS() {
	...
	before(function () {
		return request(myRequest)
			...
			.then(function (csrfResponse) {
				return EPS(csrfResponse, myRequest);
			})
			.then(function (csrfResponse) {
...
function EPS(csrfResponse, myRequest) {
	...
	myRequest.form = {
		...
		dwfrm_billing_giropayBic: 'VBOEATWWAPO'
	};
	var ExpectedResBody = {
		locale: 'en_US',
		address: {
...
describe('EPS from checkout', function () {

	describe('positive EPS', function () {
		this.timeout(60000);
		return addProductAndSubmitEPS();
	});
});
