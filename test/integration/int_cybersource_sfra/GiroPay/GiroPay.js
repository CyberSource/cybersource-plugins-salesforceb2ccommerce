...
function addProductAndSubmitGPY() {
	...
	before(function () {
		return request(myRequest)
			...
			.then(function (csrfResponse) {
				return GPY(csrfResponse, myRequest);
			})
			.then(function (csrfResponse) {
	...
	it('expect to add to cart and submit EPS to succeed', function () {
		return request(myRequest)
...
function GPY(csrfResponse, myRequest) {
...
describe('GiroPay from checkout', function () {

	describe('positive GiroPay', function () {
		this.timeout(60000);
		return addProductAndSubmitGPY();
	});
});
