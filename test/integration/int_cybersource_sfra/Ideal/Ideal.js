...
function addProductAndSubmitIdeal() {
	...
	before(function () {
		return request(myRequest)
			...
			.then(function (csrfResponse) {
				return Ideal(csrfResponse, myRequest);
			})
			.then(function (csrfResponse) {
				var order = {orderNo:'1234567'}; 
                myRequest.method = 'GET';
				var order_id = '00012901';
                myRequest.url = config.baseUrl + '/COPlaceOrder-Submit?' + 'order_id'  + '=' + '00012901' + '&' + 'provider'  + '=' + 'Ideal';
				var cookie = request.cookie(cookieString);
                cookieJar.setCookie(cookie, myRequest.order_id);
				return request(myRequest);
            });
	});
	it('expect to add to cart and submit Ideal to succeed', function () {
		return request(myRequest)
...
function Ideal(csrfResponse, myRequest) {
	...
	myRequest.form = {
		...
		dwfrm_billing_paymentMethod: 'IDL',
...
describe('Ideal from checkout', function () {

	describe('positive Ideal', function () {
		this.timeout(60000);
		return addProductAndSubmitIdeal();
	});
});
