...
function addProductAndSubmitSofort() {
	...
	before(function () {
		return request(myRequest)
			...
			.then(function (csrfResponse) {
				return Sofort(csrfResponse, myRequest);
			})
			.then(function (csrfResponse) {
	...
	it('expect to add to cart and submit Sofort to succeed', function () {
		return request(myRequest)
...
function Sofort(csrfResponse, myRequest) {
	...
	myRequest.form = {
		...
		dwfrm_billing_paymentMethod: 'SOF',
...
describe('Sofort from checkout', function () {

	describe('positive Sofort', function () {
		this.timeout(60000);
		return addProductAndSubmitSofort();
	});
});
