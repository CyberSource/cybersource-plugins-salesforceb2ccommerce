...
function buildDataToSign(params: dw.util.HashMap) {
    ...
    if (params !== null) {
        ...
        for (i = 0; i < signedFieldNames.length; i++) {
...
function AuthorizePayer(LineItemCtnrObj, paymentInstrument, orderNo) {
    var libCybersource = require('~/cartridge/scripts/cybersource/libCybersource');
    var CybersourceHelper = libCybersource.getCybersourceHelper();
    var result; var PAReasonCode; var PAVReasonCode; var AuthorizationReasonCode; var
        serviceResponse;
    var paEnabled = false;
    var Site = require('dw/system/Site');
    var CsSAType = Site.getCurrent().getCustomPreferenceValue('CsSAType').value;
