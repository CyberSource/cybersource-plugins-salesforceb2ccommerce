...
/**
 * Processes the payment form for Cybersource Alipay payments
...
function processForm(req, paymentForm, viewData) {
    var alipayFormErrors = {};

    // Alipay-specific form validation (uses SA-style form with email/phone)
    var SAForm = {
    ...
    alipayFormErrors = COHelpers.validateCreditCard(SAForm);

    // Check for validation errors
    if (Object.keys(alipayFormErrors).length) {
        return {
            error: true,
            fieldErrors: alipayFormErrors,
            serverErrors: [],
            viewData: viewData
        };
    }

    // Set up viewData for Alipay payment
    return setupAlipayViewData(req, paymentForm, viewData);
}

/**
 * Sets up viewData for Alipay payments
 * @param {Object} req - request object
 * @param {Object} paymentForm - payment form
 * @param {Object} viewData - view data object
 * @return {Object} result object with populated viewData
 */
function setupAlipayViewData(req, paymentForm, viewData) {
    ...
    // Set payment information for Alipay
...
/**
 * Saves payment information for Alipay payments
 * @param {Object} req - request object
