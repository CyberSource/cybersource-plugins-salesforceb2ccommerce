...
function validateMobilePaymentRequest() {
	...
	// Determine payment method from request headers before any order lookup
	if (headersMap.containsKey("dw_applepay_user") || headersMap.containsKey("dw_applepay_password")) {
		paymentMethod = CybersourceConstants.METHOD_ApplePay;
	} else if (headersMap.containsKey("dw_androidpay_user") || headersMap.containsKey("dw_androidpay_password")) {
	...
	// Validate credentials before performing any order lookup
	...
	if (empty(jsonObj.orderToken) || jsonObj.orderToken.length > 200) {
		Logger.error("[MobilePaymentsHelper.js] Missing or oversized orderToken in JSON");
		return {
			error: true,
	...
	var order = OrderMgr.getOrder(jsonObj.orderID, jsonObj.orderToken);
	if (empty(order)) {
		return {
			error: true,
			ERRORCODE: Resource.msg('cyb.mobilePayment.errorcode.ordernotfound', 'cybMobilePayments', null),
	...
	// Per-object authorization: verify the order's payment method matches the authenticated credential type
