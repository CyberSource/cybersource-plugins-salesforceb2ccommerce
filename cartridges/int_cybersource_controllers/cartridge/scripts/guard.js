...
function instanceValidationFailed () {
	var redirectUrl = dw.web.URLUtils.https('Home-Show');
	response.redirect(redirectUrl);
    return false;
}
...
var Filters = {
    ...
    devInstance : function() { return dw.system.System.getInstanceType() != dw.system.System.PRODUCTION_SYSTEM;}
};
...
function ensure (filters, action, params) {
    return expose(function (args) {
        ...
        var errors = [];
        params = require(CybersourceConstants.SG_CONTROLLER+'/cartridge/scripts/object').extend(params,args);
        ...
        for (var i = 0; i < filters.length; i++) {
            ...
            if (!filtersPassed) {
                ...
                } else if(filters[i] === 'devInstance') {
                	error = instanceValidationFailed;
                }
                break;
            }
        }
        ...
        if (!error) {
            error = function () {
                throw new Error('Guard(s) 22' + errors.join('|') + ' did not match the incoming request.'); 
            };
        }
